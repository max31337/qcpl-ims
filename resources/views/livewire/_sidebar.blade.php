<div class="h-full flex flex-col bg-white border-r border-border">
    ...
    <nav class="flex-1 space-y-1 p-2">
        ...
        {{-- Supply officer gets only supply-related links + my activity --}}
        @if($role === 'supply_officer')
            ...
            <a href="{{ route('supplies.analytics') }}"
               class="flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition-all hover:bg-accent hover:text-accent-foreground {{ request()->routeIs('supplies.analytics') ? 'bg-accent text-accent-foreground' : 'text-muted-foreground' }}"
               wire:navigate>
