    ...
    <x-ui-card>
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
            <div class="flex-1 flex items-center gap-3">
                <x-ui.input wire:model.debounce.300ms="search" placeholder="Search supplies (name, number, SKU)" />
                <x-ui-select wire:model="category" class="min-w-[160px]">
                    <option value="">All Categories</option>
                    @foreach($categories as $c)
                        <option value="{{ $c->id }}">{{ $c->name }}</option>
                    @endforeach
                </x-ui-select>
                <x-ui-select wire:model="status" class="min-w-[140px]">
